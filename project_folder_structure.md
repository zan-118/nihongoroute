...
11. **Pemisahan Klien Supabase Statis (Cookie-Free) untuk SSG/ISR**:
    Seluruh pengambilan data leksikal publik di tingkat Server Actions (seperti direktori pustaka dan generator sitemap) wajib menggunakan `createStaticClient()` bebas-cookie. Panggilan klien dinamis `createClient()` berbasis cookie hanya diperbolehkan untuk rute yang membutuhkan validasi sesi masuk (auth/user profile/SRS), guna menjamin kelancaran kompilasi statis Next.js (SSG/ISR) dan meniadakan galat `DYNAMIC_SERVER_USAGE`.
