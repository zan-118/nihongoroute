...
/**
 * @file enrich-sentences.mjs
 * @description Script utilitas produksi untuk memperkaya tabel `sentences` di Supabase.
 * Menerjemahkan kalimat contoh bahasa Jepang ke bahasa Indonesia (kolom indonesia) 
 * dan bahasa Inggris (kolom english) serta menentukan tingkat jlpt_level yang kosong.
 * 
 * Penggunaan:
 *   node scripts/enrich-sentences.mjs --limit 50
 */
...
function printUsage() {
  console.log(
    [
      "=== NIHONGOROUTE SENTENCES ENRICHER ===",
      "Penggunaan:",
      "  node scripts/enrich-sentences.mjs [options]",
      "",
      "Opsi:",
...
function buildPrompt(items) {
  return `
Anda adalah penerjemah bahasa Jepang profesional.
Terjemahkan kalimat contoh bahasa Jepang berikut ke bahasa Indonesia dan bahasa Inggris yang natural.
Tentukan juga estimasi tingkat JLPT yang sesuai (N5, N4, N3, N2, N1) berdasarkan kosakata dan tingkat kesulitan kalimat.
...
async function main() {
  ...
  console.log("🔍 [Database] Memuat kalimat contoh yang belum memiliki terjemahan Indonesia...");
  const { data: sentences, error } = await supabase
    .from("sentences")
    .select("id, japanese")
    .or("indonesia.is.null,english.is.null,jlpt_level.is.null")
    .limit(options.limit);
  ...
  if (!sentences || sentences.length === 0) {
    console.log("✅ Semua data sentences sudah lengkap diperkaya.");
    process.exit(0);
  }

  console.log(`📈 [Proses] Memulai enrichment ${sentences.length} kalimat contoh...`);
  
  const totalBatches = Math.ceil(sentences.length / options.llmBatchSize);

  for (let b = 0; b < totalBatches; b += 1) {
    const batch = sentences.slice(b * options.llmBatchSize, (b + 1) * options.llmBatchSize);
    console.log(`   📦 Memproses batch ${b + 1}/${totalBatches} (${batch.length} item)...`);

    const prompt = buildPrompt(batch.map((x) => ({ id: x.id, japanese: x.japanese })));
    ...
    for (const item of enrichedItems) {
      ...
      if (updateError) {
        ...
        console.log(`      └─ Sukses memperkaya ID ${item.id} ("${item.indonesia}")`);
      }
    }
  ...
  console.log("\n🎉 [Sukses] Proses enrichment kalimat selesai!");
  process.exit(0);
}
