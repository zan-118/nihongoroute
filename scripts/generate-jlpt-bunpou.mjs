...
const {
  ...
  requiresBunpouLlm,
} = await import("../src/lib/exams/bunpou-generator.ts");
...
function printUsage() {
  console.error(
    [
      "Usage:",
      "  npm run exam:generate:bunpou -- [options]",
      "",
      "Options:",
      "  --level <N5|N4|N3|N2|N1>       JLPT level. Default: N5",
      "  --limit <number>                 Maximum generated questions. Default: 100",
      "  --offset <number>                Candidate grammar offset after sorting. Default: 0",
      "  --candidate-limit <number>       Candidate grammar rows to consider. Default: auto",
      "  --pool-limit <number>            Grammar rows fetched for distractors. Default: 1000",
      "  --types <official|rule|llm|...>  Question types. Default: official",
      "  --llm-enhance                    Generate sentential_grammar_2/text_grammar with AI",
      "  --llm-limit <number>             Max AI-generated questions. Default: quota estimate",
      "  --llm-batch-size <number>        Grammar rows per AI request. Default: 4",
      ...
      "  --template-slug <slug>           Import template slug. Default: jlpt-<level>-bunpou-draft",
      "  --title <title>                  Template title. Default: JLPT <level> Bunpou Draft",
      ...
      "  --types rule",
      "  --types llm",
      "  --types sentential_grammar_1,sentential_grammar_2,text_grammar",
      "",
      "Example:",
...
function resolveQuestionTypes(options) {
  ...
  const officialTypes = getBunpouQuestionTypesForLevel(options.level);

  if (raw === "official" || raw === "all") return officialTypes;
  if (raw === "rule") {
    return officialTypes.filter((type) => !requiresBunpouLlm(type));
  }
  if (raw === "llm") {
    return officialTypes.filter((type) => requiresBunpouLlm(type));
  }
  ...
  return normalizeBunpouQuestionTypes(requested);
}
...
async function fetchGrammarRows(supabase, options) {
  ...
  const { data, error } = await supabase
    .from("grammar")
    .select(
      "id,title,meaning,slug,jlpt_level,formation,formation_furigana,formation_romaji,notes,examples,order_number"
    )
  ...
  if (error) throw new Error(`Gagal mengambil grammar: ${error.message}`);
...
function summarizeJson(value, maxLength = 360) {
...
function normalizeLlmRow(row) {
  return {
    ...
    formation_furigana: row.formation_furigana ?? null,
    notes: row.notes ? row.notes.slice(0, 520) : null,
    examples: summarizeJson(row.examples, 420),
  };
}
...
function buildLlmPrompt(input) {
  const typeGuidance = {
    ...
    text_grammar:
      "Buat soal 文章の文法: buat passage pendek level target, lalu soal grammar dari passage tersebut. Sertakan passage.key, passage.title, passage.contentHtml, dan passageKey yang sama.",
  };

  return `
Anda adalah pembuat soal JLPT profesional untuk bagian Language Knowledge (Grammar / 文法) level ${input.level}.
Target pengguna adalah pelajar Indonesia, tetapi soal, passage, dan pilihan harus bergaya JLPT dalam bahasa Jepang.
...
5. promptHtml dan passage.contentHtml harus HTML sederhana dengan tag <p>, <u>, <strong>, atau <br>. Jangan markdown.
...
8. Untuk text_grammar, passage wajib ada dan passageKey harus sama dengan passage.key.
9. Jangan membuat dua soal dengan prompt, passage, dan jawaban benar yang sama.
  ...
  "questions": [
    {
      ...
      "promptHtml": "<p>...</p>",
      "passage": {
        "key": "p-unique-key-opsional-untuk-text-grammar",
        "title": "judul pendek opsional",
        "contentHtml": "<p>...</p>",
        "sourceLabel": "AI draft"
      },
      "passageKey": "p-unique-key-opsional-untuk-text-grammar",
      "choices": ["...", "...", "...", "..."],
      "correctChoiceIndex": 0,
      "explanationHtml": "<p>Penjelasan singkat dalam Bahasa Indonesia.</p>",
      "sourceReference": "pattern / meaning"
    }
  ]
}
...
function validatePassage(rawPassage, type, sourceId) {
  if (type !== "text_grammar") return null;
  const key = compactString(rawPassage?.key) ?? `p-bunpou-${sourceId}`;
  const contentHtml = compactString(rawPassage?.contentHtml);
  if (!contentHtml) return null;

  return {
    key,
    title: compactString(rawPassage?.title),
    contentHtml,
    sourceLabel: compactString(rawPassage?.sourceLabel) ?? "AI draft",
  };
}
...
function validateEnhancedQuestions(raw, allowedTypes, targetRows, maxQuestions) {
  ...
  for (const question of questions) {
    ...
    const passage = validatePassage(question?.passage, type, sourceId);
    if (type === "text_grammar" && !passage) continue;
    ...
    accepted.push({
      ...
      passageKey:
        type === "text_grammar"
          ? passage.key
          : compactString(question?.passageKey),
      choices,
      correctChoiceIndex,
...
async function generateEnhancedQuestions(input) {
  ...
  const targetRows = stableShuffle(
    input.candidateRows.filter((row) => row.title && row.meaning),
    `${input.seed}:llm-targets`
  );
  const distractorRows = stableShuffle(
    input.grammarRows.filter((row) => row.title && row.meaning),
  ...
  for (
    ...
    const prompt = buildLlmPrompt({
      ...
      maxQuestions: Math.min(
        remaining,
        batchRows.length * input.questionTypes.length
      ),
    });
...
function assertLlmCoverage(input) {
  ...
  if (input.candidateRowsLength === 0) {
    throw new Error(
      `Gagal menghasilkan soal: Tidak ada data grammar yang terpilih (candidateRows kosong). Offset (${input.offset}) mungkin melebihi jumlah total data grammar untuk level ${input.level}.`
    );
  }
...
function printStats(report, stats, outputPath, extra = {}) {
  ...
  console.log(
    [
      `Generated bunpou draft: ${outputPath}`,
...
try {
  ...
  const grammarRows = await fetchGrammarRows(supabase, options);
  const candidateRows = options.randomize
    ? stableShuffle(grammarRows, seed).slice(0, options.candidateLimit ?? options.limit)
    : grammarRows.slice(
        options.offset,
        options.offset + (options.candidateLimit ?? grammarRows.length)
      );
