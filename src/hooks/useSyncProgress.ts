 ...
 * @description Offline-first custom hook acting as the primary orchestrator for user learning progress synchronization (Zustand & Supabase).
 ...
 * 
 * @param {Session | null} [initialSession] Optional initial auth session.
 * @returns {{ isLoading: boolean, syncNow: () => void }} Sync loading state and manual trigger function.
 * @sideEffects Triggers background Supabase RPC calls, registers auth listeners, and sets debounced timers.
 * @storeAccess Accesses and updates `useUserStore`, `useSRSStore`, and `useUIStore`.
 */
