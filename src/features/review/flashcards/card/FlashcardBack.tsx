 ...
 */
 ...
 style={{ rubyPosition: "over", rubyAlign: "space-around" }}
 >
 {isKanji ? word : docType === "sentence" ? word : (
 // Split word into kanji-furigana pairs with native ruby alignment.
 splitFurigana(word, hiraReading).map((chunk, i) => (
 chunk.furi ? (
 <ruby key={`${chunk.text}-${i}`} className="font-japanese">
 {chunk.text}
 <rt className="text-[0.55em] text-primary/80 font-bold tracking-normal select-none leading-none opacity-90">
 {chunk.furi}
 </rt>
 </ruby>
 ) : (
