 ...
 <div className="absolute top-0 right-0 w-px h-3.5 bg-primary/20 group-hover/grammar:bg-primary transition-colors duration-500" />
      ...
      <div className="space-y-5 rounded-2xl bg-card border border-border/50 dark:border-white/10 shadow-sm overflow-hidden group transition-all duration-500">
        <div className="px-6 py-4 border-b border-border flex justify-between items-center bg-primary/5">
          <div>
            <span className="text-[9px] font-bold text-primary uppercase tracking-wider px-2 py-0.5 rounded-[4px] bg-primary/10">
        ...
        <div className="px-6 pb-6 space-y-4">
          {block.content && block.content !== block.title && (
            <div className="font-japanese text-2xl font-bold text-foreground tracking-wide leading-relaxed">
              <SmartJapanese word={block.content} furigana={block.furigana} />
            </div>
          )}
          {block.translation && (
            <p className="text-sm text-muted-foreground font-medium leading-relaxed pl-4 py-2.5 rounded-r-lg whitespace-pre-wrap border-l-4 border-l-secondary/60 bg-muted/10">
          ...
          {notes && (
            <div className="mt-5 pt-4 border-t border-border/50">
              <details className="group">
                <summary className="flex items-center justify-between cursor-pointer text-xs font-bold uppercase tracking-wider text-primary hover:opacity-80 transition-opacity select-none">
          ...
          {slug && (
            <div className="mt-4 pt-4 border-t border-border/30 flex justify-end">
              <Link 
                ...
                target="_blank"
                className="text-xs font-bold uppercase tracking-wider text-primary hover:opacity-80 transition-opacity inline-flex items-center gap-1.5 select-none"
              >
