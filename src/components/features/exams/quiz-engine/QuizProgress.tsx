...
export function QuizProgress({ current, total, color = "bg-primary", indicatorClassName = "" }: QuizProgressProps) {
  ...
  return (
    <div className="w-full h-2 md:h-3 bg-[rgb(var(--background-rgb)/0.05)] rounded-full overflow-hidden relative border border-border neo-inset">
      <m.div
        initial={{ width: 0 }}
      ...
      >
        {/* Glow Tip */}
        <div className="absolute right-0 top-0 bottom-0 w-8 bg-gradient-to-l from-[rgb(var(--foreground-rgb)/0.3)] to-transparent blur-sm" />
        <div className="absolute right-0 top-0 bottom-0 w-1 bg-background shadow-[0_0_15px_rgb(var(--foreground-rgb)/0.8)]" />
      </m.div>
    </div>
  );
}