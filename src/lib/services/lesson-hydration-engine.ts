 ...
 * @description Modul dalam (Deep Module) yang mengapsulasi seluruh logika hidrasi pelajaran:
 * parsing markdown → ContentBlock, normalisasi field camelCase, hidrasi relasi paralel
 * (vocab, kanji, grammar, listening, reading), dan penyusunan blok artikel akhir.
 *
 * Interface: hydrateLessonDetail(rawLesson, fetcher) → LibraryItem
 *
 * @input  Raw DB row dari tabel `lessons` atau `articles` + implementasi LessonRelationFetcher
 * @output LibraryItem yang sepenuhnya terhidrasi dan siap render
 * @sideeffects Tidak ada — modul ini murni data-in, data-out.
 *              Efek samping I/O didelegasikan melalui interface fetcher.
 * @stores Tidak mengakses Zustand store manapun.
 */
