...
const ptComponents: PortableTextComponents = {
  ...
  types: {
    callout: ({ value }: any) => (
      <div
        className={`border-l-4 p-5 md:p-6 rounded-r-2xl my-8 relative overflow-hidden shadow-lg ${
          value.type === "warning"
...
export default async function LessonPage({ params }: Props) {
  ...
  return (
    // PERBAIKAN: pb-40 agar konten tidak tertutup navigasi bawah HP
    <main className="min-h-screen px-4 md:px-8 pt-20 md:pt-24 pb-40 bg-cyber-bg relative overflow-hidden flex flex-col">
      ...
      <article className="max-w-4xl mx-auto w-full relative z-10 flex-1">
        ...
        <header className="mb-12 md:mb-16">
          {/* PERBAIKAN: leading-tight untuk mencegah tabrakan baris teks */}
          <h1
          ...
          <div className="flex flex-col gap-10 md:gap-12 mb-16 md:mb-24">
                ...
                <div className="space-y-5 bg-cyber-surface/30 p-5 md:p-6 rounded-[2rem] border border-white/5">
                  ...
                  <div className="space-y-3">
                    {lesson.vocabList.slice(0, 8).map((v: any) => (
                      <div
                        key={v._id}
                        className="bg-cyber-bg/50 p-4 rounded-2xl border border-white/5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-0 group hover:border-amber-500/30 transition-colors"
                      >
                        <div>
                          <p className="text-white font-bold text-base md:text-lg leading-none">
                            {v.word}
                          </p>
                          <p className="text-[10px] text-amber-500/60 uppercase font-mono mt-1.5">
                            {v.romaji}
                          </p>
                        </div>
                        <div className="flex items-center gap-3 self-end sm:self-auto w-full sm:w-auto justify-between sm:justify-end">
                          <span className="text-[10px] md:text-xs font-medium text-white/50 text-left sm:text-right flex-1 sm:flex-none sm:max-w-[120px] truncate pr-2 sm:pr-0">
                            {v.meaning}
                          </span>
                    ...
                    {lesson.conversation
                      ...
                      .map((c: any, i: number) => (
                        ...
                        >
                          ...
                          <p className="text-[9px] md:text-[10px] mt-3 text-amber-500/50 not-italic border-t border-amber-500/10 pt-2 font-medium leading-relaxed">
                            {c.id}
                          </p>
                        </div>
                      ))}
            ...
            {lesson.quizzes?.length > 0 && (
              <section className="mt-6 md:mt-8">
                <h2 className="text-2xl md:text-3xl font-black text-amber-500 mb-6 md:mb-8 uppercase tracking-[0.2em] flex items-center gap-3 md:gap-4 italic drop-shadow-md border-b border-white/5 pb-4 md:pb-6 leading-tight">
                    ...
                    {lesson.examples.map((e: any, i: number) => (
                      ...
                      >
                        <div>
                          ...
                          <p className="text-xs md:text-sm text-[#c4cfde]/60 mt-1 md:mt-1.5 font-medium leading-relaxed">
                            {e.id}
                          </p>
                        </div>
                        <div className="self-end md:self-auto shrink-0 mt-2 md:mt-0">
