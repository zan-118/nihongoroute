...
export default async function CourseLevelPage({ params }: PageProps) {
  ...
  // Mode Side Quest (Eksplorasi) akan berwarna Amber, Mode JLPT Utama akan berwarna Cyan
    ...
    <main className="min-h-screen px-4 md:px-8 pt-20 md:pt-24 pb-32 bg-cyber-bg relative overflow-hidden">
      ...
      <div className="max-w-5xl mx-auto relative z-10">
        ...
        <header className="mb-12 md:mb-16 border-b border-white/5 pb-8 md:pb-10">
          ...
          <h1
            className={`text-4xl sm:text-5xl md:text-7xl font-black uppercase italic tracking-tighter drop-shadow-lg mb-3 md:mb-4 ${isSideQuest ? "text-amber-400" : "text-white"}`}
          >
            {data.category.title}
          </h1>
        ...
        {/* =========================================
            AREA LATIHAN (VOCAB, KANJI, SURVIVAL)
        ========================================= */}
        <section className="mb-16 md:mb-20">
          <div className="mb-6 flex items-center gap-3 md:gap-4">
            <h3
              className={`text-lg md:text-xl font-black uppercase tracking-widest italic border-l-4 pl-3 md:pl-4 ${themeBorder} ${themeColor}`}
            >
              Area Latihan
            </h3>
          ...
          {/* Grid disesuaikan untuk HP agar tombol tidak gepeng */}
        ...
        {/* =========================================
            MOCK EXAM SECTION (UJIAN SIMULASI)
        ========================================= */}
        {data.mockExams && data.mockExams.length > 0 && (
          <section className="mb-16 md:mb-20">
            <div className="mb-6 flex items-center gap-3 md:gap-4">
              <h3 className="text-lg md:text-xl font-black text-white uppercase tracking-widest italic border-l-4 border-amber-500 pl-3 md:pl-4 flex items-center gap-2">
        ...
        {/* =========================================
            LESSON PATH (DAFTAR BAB / SILABUS)
        ========================================= */}
        <section>
          <div className="mb-6 md:mb-8 flex items-center gap-3 md:gap-4">
            <h3
              className={`text-lg md:text-xl font-black text-white uppercase tracking-widest italic border-l-4 pl-3 md:pl-4 ${isSideQuest ? "border-amber-500" : "border-cyan-400"}`}
            >
            ...
            <div className="grid gap-3 md:gap-4">
              {data.lessons.map((lesson: any, index: number) => (
                <Link
                  ...
                  className={`group flex flex-row items-center gap-4 md:gap-6 p-5 md:p-8 rounded-2xl md:rounded-[2rem] border transition-all ${
                    isSideQuest
                ...
                >
                  <div
                    className={`w-12 h-12 md:w-14 md:h-14 shrink-0 rounded-xl md:rounded-2xl flex items-center justify-center font-black text-lg md:text-xl font-mono border shadow-inner transition-colors ${
                      isSideQuest
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="text-lg md:text-2xl font-black text-white mb-1 md:mb-2 group-hover:tracking-wide transition-all italic uppercase drop-shadow-md truncate md:whitespace-normal">
                      {lesson.title}
                    </h4>
                    {lesson.summary && (
                      <p className="text-[#c4cfde]/60 text-xs md:text-sm font-medium line-clamp-1 md:line-clamp-2">
                        {lesson.summary}
                      </p>
                    )}
                  </div>
