            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 md:gap-6">
              {jlptCategories.map((cat) => (
                <motion.div key={cat._id} variants={itemVariants} className="h-full">
                  ...
                  >
                    <Card className="flex flex-col h-full min-h-[220px] bg-card rounded-2xl p-6 md:p-8 border border-border shadow-lg cursor-pointer hover:border-primary/40 hover:bg-primary/[0.02] transition-all duration-300 group hover:shadow-xl relative overflow-hidden">
                      <span className="text-4xl md:text-5xl font-black text-foreground/10 dark:text-foreground/20 group-hover:text-primary transition-colors duration-300 mb-4 block tracking-tight">
                        {cat.title}
                      </span>
