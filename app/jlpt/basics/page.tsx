...
export default function BasicsPage() {
  ...
  const themeColor = isHira ? "text-cyber-neon" : "text-purple-400";
  const themeBorder = isHira ? "border-cyber-neon/30" : "border-purple-500/30";
  const themeBgHover = isHira
    ...
    <main className="min-h-screen bg-cyber-bg pt-24 pb-32 px-4 md:px-8 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <section className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
          <div className="bg-cyber-surface p-2 rounded-full border border-white/5 flex gap-2 shadow-[inset_0_2px_10px_rgba(0,0,0,0.5)] relative">
            <button
              onClick={() => setType("hiragana")}
              className={`relative z-10 px-8 md:px-12 py-3 rounded-full font-black uppercase tracking-widest text-[10px] md:text-xs transition-all duration-500 ${isHira ? "text-cyber-bg shadow-lg" : "text-white/40 hover:text-white"}`}
            >
              Hiragana
            </button>
            <button
            ...
            <motion.div
              ...
              className={`absolute top-2 bottom-2 w-[calc(50%-0.5rem)] rounded-full ${isHira ? "bg-cyber-neon left-2" : "bg-purple-500 right-2"} shadow-[0_0_15px_currentColor] opacity-90`}
            />
          </div>

          <div
            className={`flex items-center gap-3 px-5 py-2.5 rounded-xl bg-cyber-surface border border-white/5 text-[9px] font-black uppercase tracking-[0.2em] ${themeColor} shadow-inner transition-colors duration-500`}
          >
        ...
        <section className="bg-cyber-surface p-6 md:p-12 rounded-[3rem] border border-white/5 shadow-[15px_15px_40px_rgba(0,0,0,0.6)] overflow-x-auto custom-scrollbar relative">
          <div
                  ...
                  {row.map((char, colIndex) =>
                    char !== "" ? (
                      <motion.div
                        ...
                        className={`relative aspect-square bg-cyber-bg border border-white/5 rounded-3xl flex flex-col items-center justify-center cursor-pointer transition-all duration-300 ${themeBgHover} hover:border-current group shadow-[6px_6px_15px_rgba(0,0,0,0.5),-4px_-4px_10px_rgba(255,255,255,0.02)] active:shadow-[inset_4px_4px_10px_rgba(0,0,0,0.5)] active:scale-95`}
                      >
                        <span
          ...
          {selectedChar && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
              ...
              <motion.article
                ...
                className={`relative bg-cyber-surface p-8 md:p-12 rounded-[3.5rem] border ${themeBorder} ${themeGlow} max-w-sm w-full text-center overflow-hidden`}
              >
                <div
                ...
                <div className="relative z-10">
                  ...
                  <div className="bg-cyber-bg p-6 rounded-3xl border border-white/5 flex justify-between items-center shadow-[inset_0_2px_10px_rgba(0,0,0,0.5)]">
